function [mps_out,canon,accs,sweeps] = cheap_apply_compress(mps_in,mpo,tol,D_max,alpha,freesweeps)
% Memory efficient combination of apply and Iter_comp

%%
N = length(mps_in);

mps_out = apply(mpo,mps_in,N);
for i = N-1 : -1 : 1
    mps_out = apply(mpo,mps_out,i);
    mps_out = R_can(mps_out,i+1,tol,D_max);
end
mps_out = R_can(mps_out,1,tol,D_max);
